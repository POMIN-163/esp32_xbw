...
void ui_clear_screen(void) {
    for (int y = 0; y < 240; y++)
        for (int x = 0; x < 320; x++) vga_write_pixel(x, y, C_BLACK);
}
...
#define MAX_ITEM \
    (193 / 15)  // 193 = LCD Height (240) - header(16) - footer(16) - char
                // height (15)
...
static void ui_draw_page_list(char **files, int fileCount, int currentItem,
                              int extLen, char *title) {
    /* Header */
    ...
    if (fileCount < 1) {
        ...
        msg = "No Files Found";
        UG_PutString((320 / 2) - (strlen(msg) * 9 / 2), (240 - 16 - 13) / 2,
                     msg);
        ui_flush();
    } else {
...
char *ui_file_chooser(const char *path, const char *filter, int currentItem,
                      char *title) {
    char *result = NULL;
    int extLen = strlen(filter);
    char **files = 0;
    ...
    while (true) {
        ...
        if (fileCount > 0) {
            if (!prevKey.values[GAMEPAD_INPUT_DOWN] &&
                key.values[GAMEPAD_INPUT_DOWN]) {
                if (selected + 1 < fileCount) {
                    ++selected;
                    ui_draw_page_list(files, fileCount, selected, extLen,
                                      title);
                } else {
                    currentItem = 0;
                    ui_draw_page_list(files, fileCount, selected, extLen,
            ...
            } else if (!prevKey.values[GAMEPAD_INPUT_UP] &&
                       key.values[GAMEPAD_INPUT_UP]) {
                if (selected > 0) {
                    --selected;
                    ui_draw_page_list(files, fileCount, selected, extLen,
                                      title);
            ...
            } else if (!prevKey.values[GAMEPAD_INPUT_RIGHT] &&
                       key.values[GAMEPAD_INPUT_RIGHT]) {
            ...
            } else if (!prevKey.values[GAMEPAD_INPUT_LEFT] &&
                       key.values[GAMEPAD_INPUT_LEFT]) {
                if (page - MAX_ITEM > 0) {
                    selected = page - MAX_ITEM;
                    ui_draw_page_list(files, fileCount, selected, extLen,
                                      title);
            ...
            } else if (!prevKey.values[GAMEPAD_INPUT_A] &&
                       key.values[GAMEPAD_INPUT_A]) {
                if (fileCount < 1) {
                    vTaskDelay(10);
                    break;
                }
                ...
                size_t fullPathLength =
                    strlen(path) + 1 + strlen(files[selected]) + 1;
                ...
                result = fullPath;
                break;
            } else if (!prevKey.values[GAMEPAD_INPUT_B] &&
                       key.values[GAMEPAD_INPUT_B]) {
                vTaskDelay(10);
                break;
            }
        } else {
            if (!prevKey.values[GAMEPAD_INPUT_B] &&
                key.values[GAMEPAD_INPUT_B]) {
                vTaskDelay(10);
                break;
            }
